        ...
        [ test "(1) " <|
            \_ ->
                parseMiniLaTeX "This \\italic{good} stuff: $x^2$\n\n\\begin{theorem}\n   Pythagoras sez: $a^2 + b^2 = c^2$"
                    |> Expect.equal
                        { accumulator = Block.Accumulator.init 4
                        , ast =
                            [ Paragraph
                                [ TextM "This " { id = "0.0", label = "", loc = { begin = { col = 0, row = 0 }, end = { col = 4, row = 0 } } }
                                , ExprM "italic" [ TextM "good" { id = "0.1", label = "", loc = { begin = { col = 13, row = 0 }, end = { col = 16, row = 0 } } } ] { id = "0.1", label = "", loc = { begin = { col = 5, row = -1 }, end = { col = 17, row = -1 } } }
                                , TextM " stuff: " { id = "0.2", label = "", loc = { begin = { col = 18, row = 0 }, end = { col = 25, row = 0 } } }
                                , VerbatimM "math" "x^2" { id = "0.3", label = "", loc = { begin = { col = 26, row = 0 }, end = { col = 30, row = 0 } } }
                                , TextM "\n" { id = "0.4", label = "", loc = { begin = { col = 31, row = 0 }, end = { col = 31, row = 0 } } }
                                ]
                            ...
                            , Block "theorem"
                                [ Paragraph
                                    ...
                                    { begin = 3, end = 3, id = "3", indent = 3, status = BlockComplete }
                                ]
                                { begin = 2, end = 3, id = "0", indent = 0, status = BlockUnfinished "begin" }
                            ]
                        }
        ]
